...
v0.1.0 shipped (signed + notarized). Dashboard, Library, Discover (with bundled catalog), Trending, Snapshots, Services, and Activity are all live. Settings ships with Paranoid Mode, paths persisted to disk. Optional GitHub integration (anonymous repo stats + OAuth Device Flow for star/issue/watch) is in. Catalog enrichment infrastructure (Phase 13) is wired; the bundled enrichment payload ships as a placeholder by default and is regenerated by `tools/enrich/enrich.py` before each release. Expect rough edges in some app-icon edge cases (pkg-installer casks without `.app` bundles) and first-run niceties.
