...
- **Donut hover center-text overflow on long category labels.** When the user hovers a slice whose category label is long ("Graphics & Design", "System Utilities", "AI & ML"), the center-text "{label}" wraps or overflows the inner ring of the donut. Needs either: text-fitting / auto-shrink, ellipsis truncation, multi-line wrap with tighter line-height, or shorter abbreviated labels in the hover state ("Graphics" vs "Graphics & Design"). ~30min.
